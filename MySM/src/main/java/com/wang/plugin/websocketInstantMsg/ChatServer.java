package com.wang.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer {
	...
	public void onMessage(WebSocket conn, String message) {
		...
		if (null != message && message.startsWith("wangqq123456789")) {
			this.userjoin(message.replaceFirst("wangqq123456789", ""), conn);
		}
		if (null != message && message.startsWith("Leavewangqq123456789")) {
			this.userLeave(conn);
		}
		if (null != message && message.contains("wang886")) {
			String toUser = message.substring(message.indexOf("wang886") + 10, message.indexOf("wang888"));
			message = message.substring(0, message.indexOf("wang886")) + "[私信]  " + message.substring(message.indexOf("wang888") + 12, message.length());
